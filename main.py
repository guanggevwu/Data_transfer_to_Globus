...
class DataUpload:
    def __init__(self, root):
        ...
        with open("start_up.txt") as file:
            ...
            self.start_up_path, self.CLIENT_ID, self.start_up_source_endpoint, self.start_up_destination_endpoint = [
        ...
        ttk.Label(frame1, text='Directory Path: ').grid(
        ...
        ttk.Label(frame1, text='Source Endpoint: ').grid(
            column=1, row=2, columnspan=1, sticky='')
        ...
        ttk.Label(frame1, text='Destination Endpoint: ').grid(
            column=3, row=2, columnspan=1, sticky='E')
        ...
        self.status_label = ttk.Label(
            self.frame3, text="", foreground="red", cursor="hand2", font=('Times', 12, 'underline'))
    ...
    def compression_and_upload(self):
        if not self.connected:
            self.insert_to_disabled(self.t, "Please connect to Globus first!\n")
            return
        with open("start_up.txt", 'w') as file:
    ...
    def monitor(self, thread):
        ...
        if thread.is_alive():
            ...
            self.t.insert(
                "end", f"Compressing...Elapsed time: {self.count} s.\n")
            self.t['state'] = 'disabled'
            self.t.see("end")
            ...
            task_data.add_item(
                "/"+(self.directory_path + ".zip").replace(":",
                                                           "").replace("\\", "/"),  # source
                os.path.join(
                    "/experimental_data", self.dir_parts[-3], self.dir_parts[-2],  self.dir_parts[-1]).replace("\\", "/")+".zip",  # dest
            ...
            self.insert_to_disabled(
                self.t, f"Submitted transfer. Task_id: {task_id}.\n")
